...
def login():
    ...
    try:
        ...
        print(e)
        print("[bold red]alert![/bold red] Please check your network connection and try again!")
        s.quit_driver()
...
def scrape(
    ...
    location: str = typer.Option(None, "--location", "-l", help="Use it if you want to search a particular location e.g Nigeria"),
    ...
    try:
        with spinner("Accessing linkedin with your credentials and scrapping data..."):
            links= s.scrape_linkedin_job_links(search_params=search, country_filter_param=location)
            print(links)
            for link in links:
...
def main(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        print("[bold yellow]Usage:[/bold yellow] main.py [OPTIONS] COMMAND [ARGS]...\n[light grey]Try[/light grey] 'main.py --help' [light grey]for help.[/light grey]")
